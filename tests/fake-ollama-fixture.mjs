...
// TODO(phase-3): replace installFakeOllama with a fake Ollama HTTP server fixture
export function installFakeOllama(binDir, behavior = "review-ok") {
  const statePath = path.join(binDir, "fake-ollama-state.json");
  // TODO(phase-3): rename fake binary from "codex" to "ollama" when switching backends
...
if (args[0] === "--version") {
  // TODO(phase-3): update version string when switching to Ollama binary
  ...
  try {
    switch (message.method) {
      case "initialize":
        ...
        saveState(state);
        send({ id: message.id, result: { userAgent: "fake-ollama-app-server" } });
        break;
      ...
      case "review/start": {
        ...
        const payload = message.params.outputSchema && message.params.outputSchema.properties && message.params.outputSchema.properties.verdict
          ...
          : taskPayload(prompt, thread.name && thread.name.startsWith("Ollama Companion Task") && prompt.includes("Continue from the current thread state"));
...
`;
  ...
  // TODO(phase-3): rename codex.cmd to ollama.cmd when switching backends
