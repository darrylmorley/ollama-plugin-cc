...
import { getSessionRuntimeStatus } from "./ollama.mjs";
...
function inferLegacyJobPhase(job, progressPreview = []) {
  ...
  for (let index = progressPreview.length - 1; index >= 0; index -= 1) {
    ...
    if (line.startsWith("starting ollama") || line.startsWith("thread ready") || line.startsWith("turn started")) {
      return "starting";
    }
    ...
    if (line.startsWith("ollama error:") || line.startsWith("failed:")) {
      return "failed";
    }
  }
...
export function resolveResultJob(cwd, reference) {
  ...
  throw new Error("No finished Ollama jobs found for this repository yet.");
}
...
export function resolveCancelableJob(cwd, reference, options = {}) {
  ...
  if (sessionScopedActiveJobs.length > 1) {
    throw new Error("Multiple Ollama jobs are active. Pass a job id to /ollama:cancel.");
  }

  if (getCurrentSessionId(options)) {
    throw new Error("No active Ollama jobs to cancel for this session.");
  }

  throw new Error("No active Ollama jobs to cancel.");
}
